...
import { sanitizePublicAddress, containsIpOrPort } from '../lib/networkUtils';
...
export const DestinationModal: React.FC<DestinationModalProps> = ({ app, onClose, currentUser }) => {
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                {isUnc ? 'اشتراک شبکه / مسیر UNC' : 'فایل و منبع محلی سیستم'}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="py-5 space-y-4">
          <div className="p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs flex gap-3 items-start">
            ...
            <div className="space-y-1">
              <p className="font-bold text-amber-200">
                راهنمای پروتکل امنیتی مرورگر
              </p>
              <p className="text-amber-300/80 leading-relaxed text-[11px]">
                مرورگرهای وب برای حفظ امنیت از باز کردن مستقیم مسیرهای اشتراک شبکه (UNC / SMB) جلوگیری می‌کنند. مسیر زیر را کپی کرده و در نوار آدرس مدیریت فایل سیستم خود قرار دهید.
              </p>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              {currentUser?.role === 'admin' && containsIpOrPort(app.url) && (
                <button
                  type="button"
                  onClick={() => setShowRaw(!showRaw)}
                  className="text-[11px] font-medium text-blue-400 hover:underline cursor-pointer"
                >
                  {showRaw ? 'پنهان‌سازی مسیر کامل' : 'نمایش مسیر کامل ادمین'}
                </button>
              )}
            </div>
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                dir="ltr"
                value={
                  currentUser?.role === 'admin' && showRaw
                    ? app.url
                    : sanitizePublicAddress(app.url, currentUser?.role)
                }
              ...
              <button
                ...
                onClick={handleCopy}
                title="کپی کردن آدرس مقصد در کلیپ‌بورد"
            ...
            {(!currentUser || currentUser.role !== 'admin') && containsIpOrPort(app.url) && (
              <p className="text-[10px] text-slate-400 mt-1">
                🔒 هاست و پورت در نمای عمومی محافظت شده است. عملیات کپی مسیر قابل استفاده را کپی می‌کند.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-white/[0.1] flex items-center justify-between">
          <span className="text-[11px] text-slate-400 font-mono">
            {app.isPublic ? 'دسترسی عمومی' : 'دسترسی خصوصی'}
          </span>
          <button
            onClick={onClose}
